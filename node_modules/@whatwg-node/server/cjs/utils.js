...
function pumpToWritable(source, dest, isClosed) {
    const pump = () => (0, promise_helpers_1.handleMaybePromise)(source, sourceResult => {
        if (isClosed?.() || sourceResult.done) {
            return endDest(dest);
        }
        return (0, promise_helpers_1.handleMaybePromise)(() => safeWrite(sourceResult.value, dest), () => (isClosed?.() ? endDest(dest) : pump()));
    });
    return pump();
}
...
function sendNodeResponse(fetchResponse, serverResponse, nodeRequest, __useSingleWriteHead) {
    ...
    if (fetchResponse['bodyType'] === 'String') {
        ...
        return (0, promise_helpers_1.handleMaybePromise)(() => safeWrite(bodyString, serverResponse), () => endResponse(serverResponse));
    }
